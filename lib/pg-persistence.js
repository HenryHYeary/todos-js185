...
module.exports = class PgPersistence {
  ...
  async createNewTodo(todoListId, todoTitle) {
    const CREATE_NEW_TODO = "INSERT INTO todos (title, todolist_id) VALUES ($2, $1)";

    let result = await dbQuery(CREATE_NEW_TODO, todoListId, todoTitle);
    return result.rowCount > 0;
  }
  ...
  async editTodoListTitle(todoListId, todoListTitle) {
    const EDIT_LIST_TITLE = "UPDATE todolists SET title = $1 WHERE id = $2";

    let result = await dbQuery(EDIT_LIST_TITLE, todoListTitle, todoListId);
    return result.rowCount > 0;
  }
  ...
  async createNewTodoList(title) {
    const CREATE_NEW_TODOLIST = "INSERT INTO todolists (title) VALUES ($1)";

    try {
      let result = await dbQuery(CREATE_NEW_TODOLIST, title);
      return result.rowCount > 0;
    } catch (error) {
